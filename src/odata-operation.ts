...
export abstract class ODataOperation<T> {
  ...
  constructor(protected _typeName: string, protected config: ODataConfigService, protected http: Http) {}
...
export abstract class OperationWithKey<T> extends ODataOperation<T> {
  constructor(
      protected _typeName: string, protected config: ODataConfigService, protected http: Http, protected key: string) {
...
export abstract class OperationWithEntity<T> extends ODataOperation<T> {
  constructor(
      protected _typeName: string, protected config: ODataConfigService, protected http: Http, protected entity: T) {
    super(_typeName, config, http);
  }
  abstract Exec(...args): Observable<any>;
}
...
//         return this.handleResponse(this.http.post(this.config.baseUrl +
//         "/"+this._typeName, body, this.getRequestOptions()));
//     }
// }
...
//         let body = JSON.stringify(this.entity);
//         return
//         this.http.patch(this.getEntityUri(this.key),body,this.getRequestOptions());
//     }
// }
