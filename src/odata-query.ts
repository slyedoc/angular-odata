...
export class ODataQuery<T> extends ODataOperation<T> {
  ...
  constructor(_typeName: string, config: ODataConfigService, http: Http) { super(_typeName, config, http); }
  ...
  private buildResourceURL(): string { return this.config.baseUrl + '/' + this._typeName + '/'; }
